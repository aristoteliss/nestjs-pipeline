...
6. Mutate aggregates through factories/domain methods, not direct setters or synthetic snapshots constructed only to trigger persistence. Aggregates inherit from our owned, framework-neutral `AggregateRoot` (NestJS 12 semantics in `@nestjs-pipeline/ddd-core/domain`). Public setters on aggregates exist strictly for MikroORM hydration (`accessor: true`), are annotated `@internal`/`@deprecated`, and direct assignments in application/CQRS/controllers code are forbidden at build time by `biome/plugins/aggregate-identity.grit`.
...
14. Persistence lifecycle structure, aggregate encapsulation, and standalone package isolation are guarded by Biome Grit plugins (`biome/plugins/persistence-lifecycle.grit`, `aggregate-identity.grit`, `package-licenses.grit`, `verify-package-licenses.grit`, `test-suite.grit`); verify with `pnpm lint:persistence` and `pnpm check`.
