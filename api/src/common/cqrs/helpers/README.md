...
Repository cache keys use one deterministic JSON serializer: `stableStringify` from `@cqrs-ddd/safe-stringify`, the same one `@nestjs-pipeline/core` uses. The tenant comes from `@cqrs-ddd/core`'s tenant scope (`runWithTenant`), which `TenantScopeBehavior` (`@nestjs-pipeline/tenant`) sets from the pipeline tenant for every command, query and event. `filterCacheKey` owns only repository-specific concerns such as tenant namespacing, top-level filter segments and escaping the `:` / `\\` delimiters used by its key format. Do not add another recursive object sorter or JSON canonicalizer in this application.
