...
Repository cache keys use one deterministic JSON serializer: `stableStringify` from `@cqrs-ddd/safe-stringify`, the same one `@nestjs-pipeline/core` uses. The tenant comes from the resolver `ObservabilityModule` registers with `setTenantResolver`: `currentTenantId` from `@nestjs-pipeline/tenant`, the tenant of the running pipeline or `runWithTenant` scope. `filterCacheKey` owns only repository-specific concerns such as tenant namespacing, top-level filter segments and escaping the `:` / `\\` delimiters used by its key format. Do not add another recursive object sorter or JSON canonicalizer in this application.
