...
1. extend `MikroOrmWriteSideCommandRepository<TSnapshot, TEntity, TResult>` from `@cqrs-ddd/core/persistence`, passing `MikroOrmStore` as its `IEntityManagerSource` and `hydrateFn: (s) => TEntity.fromJSON(s)`;
...
Command repositories across `users-api` apply declarative lifecycle decorators and helpers from `@cqrs-ddd/core/persistence`:
...
`@cqrs-ddd/core/persistence`. Shared implementations and their lifecycle contract live in
[`ddd-core` persistence documentation](../../../packages/ddd-core/README.md#decorated-versioned-updates).
...
`@cqrs-ddd/core/persistence`; renamed imports are rejected by the plugin.
...
and Biome plugin tests live in `packages/ddd-core/persistence`.
...
`MikroOrmCache` comes from `@cqrs-ddd/core/persistence`; `persistence.module.ts`
