...
`UserLoginService.signToken()` resolves `UserCapabilities` through `USER_CAPABILITY_READER` (implemented by `GetUserCapabilitiesQueryRepository`). The standalone `GetUserCapabilitiesQuery` handler uses the same repository port when the capability lookup is invoked as a top-level CQRS query.
