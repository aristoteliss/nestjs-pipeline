...
`UserLoginService.signToken()` resolves `UserCapabilities` directly through `QUERY_REPOSITORY.getUserCapabilities` (`GetUserCapabilitiesQueryRepository`). The standalone `GetUserCapabilitiesQuery` handler uses the same repository port when the capability lookup is invoked as a top-level CQRS query.
