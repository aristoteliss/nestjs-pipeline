...
describe('aggregate construction contracts', () => {
  ...
  it('keeps aggregate constructors private and ORM compatibility casts in persistence only', () => {
    ...
    expect(source('../../persistence/schemas/user.schema.ts')).toContain(
      'class: User as any',
    );
    expect(source('../../persistence/schemas/role.schema.ts')).toContain(
      'class: Role as any',
    );
    expect(source('../../persistence/schemas/auth.schema.ts')).toContain(
      'class: Auth as any',
    );
  });
});
