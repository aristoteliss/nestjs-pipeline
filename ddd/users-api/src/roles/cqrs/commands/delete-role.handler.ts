...
@UsePipeline(
  ...
  [CaslBehavior, { rules: [{ action: APP_ACTIONS.DELETE, subject: APP_SUBJECTS.ROLE }] }],
  [ResilienceBehavior, { handle: isTransientPersistenceError, retry: { maxAttempts: 3, backoff: { type: 'exponential', initialDelay: 100, maxDelay: 2_000 } } }],
  [AuditBehavior, { action: AUDIT_ACTIONS.ROLE_DELETE, severity: AUDIT_SEVERITY.HIGH, actor: () => { const sessionUser = getSessionUserFromStore(); return sessionUser ? { id: sessionUser.id, email: sessionUser.email ?? undefined } : undefined; } }],
)
export class DeleteRoleHandler extends CommandBaseHandler<DeleteRoleCommand, Role> {
  constructor(
    @Inject(COMMAND_REPOSITORY.deleteRole)
    private readonly commandRepository: IWriteSideAggregateRepository<Role, RoleSnapshot, null>,
