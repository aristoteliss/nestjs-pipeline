  ...
  implements IWriteSideAggregateRepository<Role, RoleSnapshot, RoleSnapshot>
{
  ...
  @Cache<Role, RoleSnapshot>((role) => filterCacheKey(Role.aggregateName, { id: role.id }))
  async save(role: Role): Promise<RoleSnapshot> {
    try {
      const affected = await this.store.em.nativeUpdate(Role, { id: role.id, version: role.getExpectedVersion() }, { name: role.name, updatedAt: role.updatedAt, version: role.version });
      if (affected === 0) {
        const exists = await this.store.em.findOne(Role, { id: role.id }, { refresh: true });
        if (exists) throw OptimisticLockError.lockFailedVersionMismatch(role, role.getExpectedVersion(), exists.version);
        throw new NotFoundException('Role not found');
      }
      return role.toJSON();
    } catch (err: unknown) {
      if (err instanceof UniqueConstraintViolationException || (typeof err === 'object' && err !== null && 'code' in err && err.code === 'SQLITE_CONSTRAINT_UNIQUE') || (err instanceof Error && (err.message.includes('UNIQUE') || err.message.includes('unique') || err.message.includes('SQLITE_CONSTRAINT_UNIQUE')))) {
