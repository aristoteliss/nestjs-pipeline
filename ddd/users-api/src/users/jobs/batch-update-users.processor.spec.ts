...
describe('BatchUpdateUsersProcessor tenant isolation', () => {
  it('rejects a mixed-tenant batch before entering TenantSchemaContext', async () => {
    const run = vi.fn();
    const tenantContext = { run, schema: 'tenant_a' } as unknown as TenantSchemaContext;
    ...
    await expect(
      processor.process({ data, opts: { correlationId: 'corr-mixed' } } as Job<BatchUpdateUserItem[]>),
