...
| N-01 | P1 | A composed, cached read model returns user fields with no entity or field authorization. | ddd/users-api/src/users/cqrs/queries/get-user-overview.handler.ts, user-overview-cache.policy.ts | **Partially done.** The loaded user is authorized (`authorize('read', user, { select })`) and the composed candidate is projected against it (`project`), so entity, field and descendant-array masks apply; roles are kept only when the loaded `Role` passes `read` and `read name`; the user is loaded with `{ refresh: true }`; the response cache is partitioned by tenant, principal type/ID from the CASL user context, rule digest and policy version, and bypassed for conditional `read` rules on `User`, `Role` or `all`. **Remaining:** additional capabilities are exposed under the `capabilities` field of the User read rule with no separate policy; the freshness regression through a real repository-cache adapter is not present; response-cache freshness for conditional related resources rests on the bypass, not on invalidation. | Define an explicit policy for reading another user's additional capabilities (fail closed until it exists), add the repository-cache freshness regression, then close. |
...
| 4 | **N-01** | Ασφάλεια (P1) | Σύνθετο cached read model χωρίς entity/field authorization | **Μερικώς ολοκληρώθηκε.** Έχουν υλοποιηθεί: `authorize('read', user, { select })` και `project` πάνω στο φορτωμένο aggregate, έλεγχος `Role` πριν από την έκθεση ονομάτων, ανάγνωση με `refresh: true`, partitioned cache key και παράκαμψη cache για conditional `read` κανόνες. Απομένουν: ρητή πολιτική για additional capabilities και regression φρεσκάδας με πραγματικό repository-cache adapter. | Open. |
