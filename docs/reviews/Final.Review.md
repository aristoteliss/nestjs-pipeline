...
| N-03 | P1 | Audit actor for login is taken from the caller-supplied request body, and now defeats the `authenticated` discriminator the rest of the audit trail carries. | ddd/users-api/src/auths/cqrs/commands/create-auth.handler.ts:42-46 | **Open, reproduced (R).** The actor factory returns `{ id: req?.email ?? 'anonymous', email: req?.email }` from `CreateAuthCommand`. Executing the command through the real `CommandBus` with `AuditModule.forRoot({ defaults: AUDIT_MODULE_DEFAULTS })` emits `actor = {"id":"attacker@evil.test","email":"attacker@evil.test"}` — an `id` from the request body and no `authenticated` field. Since F-06 gave every other audited handler `{ authenticated: true \| false }`, a consumer filtering `actor.authenticated === false` no longer excludes login records, and a consumer reading `actor.id` cannot separate a real principal from a failed login under an administrator's address. Raised from P2 because F-06 introduced the discriminator this record silently lacks. | Record the claimed identity as a claimed subject, not as `actor.id`: `{ authenticated: false, claimedEmail: … }`. Nothing is lost — `record.payload.email` already carries the attempted address. Drop the `'anonymous'` fallback. Add an emitted-record assertion for `CreateAuthHandler`, which `deletion-audit-records.spec.ts` does not cover. See `docs/reviews/Audit.Trusted.Actor.Review.md`. |
...
F-05, F-07 and F-11 are reproduced defects on this branch and are the next implementation targets, together with N-01. F-04, F-09, F-14, F-16 and F-17 remain open from code-path evidence. N-02 through N-06 are new findings from this review.
