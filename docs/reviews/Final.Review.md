...
| 4 | ~~**N-01**~~ | Ασφάλεια (P1) | Σύνθετο cached read model χωρίς entity/field authorization | **Ολοκληρώθηκε.** Κλήση `authorizer.authorize('read', user)` πριν τη σύνθεση του DTO, explicit allowlist προβολής πεδίων (διατήρηση `null` vs `undefined`), αποφυγή περιττών queries για ρόλους/capabilities αν δεν επιτρέπονται, partitioned cache key με tenant, principal `${type}:${id}`, scope digest (κανόνες + capabilities) με policy version `v2`, παράκαμψη cache όταν υπάρχουν entity conditions. Καλύφθηκε με unit tests και integration tests (απομόνωση διαφορετικών principals, ανανέωση scope, fail-closed σε ελλιπές context). | Closed. |
