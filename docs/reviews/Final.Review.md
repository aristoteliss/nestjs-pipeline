...
| F-01 | P2 (was P1) | Repository cache coordination: atomic fill fencing, safe keys, explicit consistency. | ddd/core/persistence/cache.interface.ts:98-134; decorators/FromCache.ts:144-198 (versioned) and :200-283 (legacy); helpers/filter-cache-key.helper.ts; ddd/users-api/src/persistence/cache/mikro-orm.cache.ts | **Mostly done.** `IVersionedCache` with `readState`/`tryFill`/revision fencing exists, `MemoryCache` and `MikroOrmCache` implement it, and filter keys are a SHA-256 canonical `[tenant, resource, normalizedFilter]` tuple that preserves value types and fails closed without tenant. **Residual:** an adapter that implements only `get`/`set` silently falls through to the legacy path, which still writes a stale snapshot over a deletion barrier installed between its post-DB `get()` (FromCache.ts:215) and its `set()` (:236/:278). | Decide explicitly what an unversioned adapter may do: reject it at configuration time, or bypass both reads and fills for that repository path. Silently degrading to the racy path contradicts the capability-check requirement. Keep the adapter-contract tests and add one for "unversioned adapter supplied". |
...
| F-04 | P1 | Idempotency replay is not bound to the authorization scope. | ddd/users-api/src/users/cqrs/commands/create-user.handler.ts:26-32; roles/cqrs/commands/create-role.handler.ts:23-31 | **Open.** No `replayScope` exists in the package or the sample. Keys are `tenant:actor:action:business-id` with an `'anonymous'` fallback, so unauthenticated callers share one namespace and a replay skips the handler's entity checks after a permission change. | Add an optional `replayScopeFactory` plus a stored `replayScope` preserved by Memory/Redis/Postgres stores; make it mandatory in the protected create handlers; compare before returning any completed response and fail closed on mismatch or a legacy record. Resolve the principal from trusted session context, never from a body field, and never from `'anonymous'`. Keep the operation key stable — rotating it on permission change re-enables the side effect. |
| F-05 | P1 | Mutation events carry the pre-mutation version and timestamp. | ddd/core/domain/decorators/Mutate.ts:31-52; ddd/core/domain/models/root.entity.ts:301-311; ddd/core/domain/events/root-domain.event.ts:174-196; ddd/users-api/src/users/domain/models/user.entity.ts:137-170; roles/domain/models/role.entity.ts:92-104 | **Open, reproduced (R).** `@Mutate` runs the method first and `onUpdate()` afterwards, but the method has already constructed the event, and `RootDomainEvent`'s constructor snapshots `entity.version` and `entity.toJSON()` at that moment. After `user.update(...)` the aggregate is version 2 while the event reports version 1 and the earlier `updatedAt`. | Order concrete mutations explicitly: validate, apply state, run the update lifecycle once, then construct and apply the event from the resulting snapshot. Remove `@Mutate` from those methods so no second increment happens; inventory other consumers before changing the decorator's export. Preserve `getExpectedVersion()` as the persisted baseline and do not change delete concurrency semantics. Assert event version equals aggregate version after every successful mutation, and that rejected mutations emit nothing. |
| F-06 | P1 | Audit configuration that looks correct and is ignored. | ddd/users-api/src/users/cqrs/commands/delete-user.handler.ts:38; roles/cqrs/commands/delete-role.handler.ts:38; packages/pipeline-audit/src/interfaces/audit-options.interface.ts:92; helpers/build-record.ts:43 | **Open, reproduced (R).** `AuditBehaviorOptions` declares `metadata`; both HIGH-severity delete handlers pass `metadataFactory`. The behaviour tuple accepts any `Record<string, unknown>`, so it compiles. The emitted record's metadata is `{tenantId}` only — target user, acting user and acting email are all lost, and no actor factory is configured for these handlers. | Rename to `metadata` in both handlers using the declared callback type. Configure a trusted actor factory (preferably once in the application `AuditModule` defaults). Extract option literals as constants with `satisfies AuditBehaviorOptions` so an unknown property is a compile error. Test the emitted record, not the presence of a decorator entry. |
| F-07 | P1 | Hand-written deep equality returns the wrong answer for Sets. | packages/pipeline-zod/src/helpers/zod-data.helpers.ts:133-146 | **Open, reproduced (R).** The Set branch uses a nested `for` loop without consuming the matched element, so two left entries may both match one right entry. The Map branch directly above does consume. `Set([{n:1},{n:1}])` compares equal to `Set([{n:1},{n:2}])`, and the comparison is asymmetric: the reverse direction returns `false`. Equality decides whether a mutated command is revalidated. | Apply the Map branch's one-to-one consumption principle to Sets. Keep the existing supported-value semantics; do not substitute JSON stringification. Test unequal multiplicities, insertion-order independence, nested Sets/Maps and symmetry, plus a behaviour-level regression where a Set entry is mutated without changing the Set size and the pipeline must revalidate and reject. |
...
| F-09 | P1 transaction contract, P2 duplication | Duplicated optimistic delete and an inconsistent transaction contract. | ddd/users-api/src/users/persistence/delete-user.command-repository.ts; roles/persistence/delete-role.command-repository.ts; ddd/core/persistence/optimistic-update.ts:80-83 | **Open (S).** Neither `optimisticDelete` nor `assertAutocommit` exists. The two 68-line delete repositories are identical apart from one secondary cache key (`name` vs `email`). `optimisticUpdate` rejects an outer transaction; the delete and create paths do not, so a transaction-scoped EntityManager can trigger acknowledgment, cache eviction and event publication before commit. | Add `assertAutocommit(em, operation)` beside `optimisticUpdate` and reuse it in update, the new `optimisticDelete`, and any create/save whose successful return is treated as durable. Add `optimisticDelete` owning the conditional delete on `{id, version: getExpectedVersion()}`, the refreshed diagnostic read, and the missing/conflict distinction. Capture the EntityManager once per operation. Do not apply a version predicate to unversioned Auth rows. |
...
| F-16 | P2 | Dead-letter queue receives expected rejections. | ddd/users-api/src/infrastructure/reliability.module.ts:64-82 | **Partially done.** `ignoreErrors` now covers `ZodValidationError`, `UnauthorizedException` and `UnauthorizedActionException`. It still does not cover the framework-neutral business rejections the application actually raises — `EntityNotFoundException`, unique-constraint exceptions, `EmptyUserUpdateException`, `InvalidLoginCredentialsException`, `ConcurrencyConflictError`, feature-disabled, rate-limit and idempotency conflicts — so an ordinary duplicate-email signup still produces a dead letter with a retained payload. | Define an application-owned classification near the composition root, keyed on explicit error types, not message substrings. Inventory concrete classes rather than excluding every `DomainException` subclass. Treat `IdempotencyCompletionError` with `executionSucceeded` as post-success operational failure, never as replayable work. Assert that expected rejections create zero DLQ records and that an unexpected processor failure creates exactly one. |
...
| N-01 | P1 | A composed, cached read model returns user fields with no entity or field authorization. | ddd/users-api/src/users/cqrs/queries/get-user-overview.handler.ts:83-105, compared with cqrs/queries/get-user.handler.ts:32-35 | **New (S).** `GetUserHandler` loads the aggregate and returns `authorizer.authorize('read', user)`, applying entity and field-level rules. `GetUserOverviewHandler` is reachable under the same type-level rule `read User`, loads the same aggregate through the same repository, and returns `username`, `email` and `department` directly with no `CaslAuthorizer` call. The result is then cached for 60 s, so a hit also skips any future check. This contradicts AGENTS.md rule 4 and architecture-skill rule 7 in the handler the skill's own comment presents as the exemplary cache consumer. | Authorize the loaded aggregate before composing the DTO, exactly as `GetUserHandler` does, and derive the returned fields from the authorized projection. Keep the partitioned cache key. Add a regression asserting that two principals with different field permissions receive different overview payloads, and that a cache hit cannot cross that boundary. |
...
| N-03 | P2 | Audit actor for login is taken from the caller-supplied request body. | ddd/users-api/src/auths/cqrs/commands/create-auth.handler.ts:45-49 | **New (S).** The `AuditBehavior` actor factory returns `{ id: req?.email ?? 'anonymous', email: req?.email }` from `CreateAuthCommand`. The audit trail therefore attributes a login attempt to an unverified, attacker-chosen identity, and failed attempts are recorded under whatever string was submitted. | For a pre-authentication command the claimed identity is the only one available; record it as a claimed subject rather than as `actor.id`, or emit the actor only once authentication succeeds. Do not let a body field populate an identity field that downstream consumers read as authenticated. Pair with F-06's trusted actor factory work. |
...
| S-06 | P1 | `delete-user.command-repository.ts` and `delete-role.command-repository.ts` are identical apart from one secondary cache key line; no `optimisticDelete` exists. | A safety-critical concurrency algorithm is copied into each delete repository. | Add `optimisticDelete(em, Entity, aggregate, entityName)` beside `optimisticUpdate`. See F-09, which additionally requires a shared `assertAutocommit`. | Callers can still issue custom deletes for non-versioned or special cases. | **No — additive.** Migrated repositories must preserve exact delete/conflict semantics. |
| S-07 | P1 | `createPartitionedRateLimitKeyFactory` and `createPartitionedCacheKeyFactory` exist; users-api still builds `${tenantId}:${actorId}:user.create:${request.email}` by hand (create-user.handler.ts:26-37, create-role.handler.ts:23-31, create-auth.handler.ts:29-32). No partitioned idempotency helper exists. | Security-sensitive key construction is duplicated application code, including delimiter composition and an `'anonymous'` fallback. This is the category where small mistakes become cross-tenant bugs. | Use the existing partitioned rate-limit helper in users-api and add a symmetric `createPartitionedIdempotencyKeyFactory` with explicit tenant/principal/request identity and fail-closed defaults. Cache keeps its own helper because it additionally needs permission-scope semantics. | Arbitrary `keyFactory` remains available for genuinely unusual key semantics. | **Operational key-namespace change possible.** Migrating a deployed key factory produces different canonical keys, resetting limiter buckets and idempotency records. Version or explicitly accept the reset. |
...
| A-07 | create-user/create-role/create-auth key factories | Replace manual tenant-delimited strings with the partitioned helpers, and remove the `'anonymous'` principal fallback. | Existing package helpers own escaping and missing-context policy; the fallback merges distinct unauthenticated callers into one namespace. | **Operational key-namespace change possible.** Existing limiter/idempotency keys change format. Preserve or version the format if continuity matters. |
...
E-01 and E-02 are implemented. F-02, F-03, F-08 and F-10 are implemented, and F-01 is implemented except for its unversioned-adapter policy. S-01, S-02 and S-15 are shipped; A-04 and U-09 are closed; S-08, A-01 and U-10 are superseded by a real pipeline-cache consumer.

F-05, F-06, F-07 and F-11 are reproduced defects on this branch and are the first implementation targets, together with N-01. F-04, F-09, F-14, F-16 and F-17 remain open from code-path evidence. N-02 through N-06 are new findings from this review.
...
| 1 | **F-06** | Ορθότητα (P1) | Audit options που αγνοούνται σιωπηλά | Αναπαραγώγιμο σφάλμα με άμεσο regression· χάνεται το μεταδεδομένο διαγραφής χρήστη σε εγγραφή severity high. | Μετονομασία σε `metadata`, ρύθμιση έμπιστου actor factory, `satisfies AuditBehaviorOptions` στα literals και έλεγχος της παραγόμενης εγγραφής. |
| 2 | **F-07** | Ορθότητα (P1) | Λανθασμένη ισότητα Sets στο Zod fast path | Αναπαραγώγιμο και ασύμμετρο· επηρεάζει την απόφαση επανελέγχου μετά από mutation. | Κατανάλωση ενός προς ένα όπως ήδη κάνει ο κλάδος Map· tests πολλαπλότητας, σειράς και συμμετρίας. |
| 3 | **F-05** | Ορθότητα (P1) | Events με προηγούμενο version και timestamp | Αλλάζει τη μορφή των domain methods, άρα προηγείται των S-03/S-04 που αγγίζουν τις ίδιες διαδρομές. | Ρητή σειρά: έλεγχος, αλλαγή κατάστασης, lifecycle μία φορά, κατασκευή event από το τελικό snapshot. Αφαίρεση `@Mutate` από τις συγκεκριμένες μεθόδους. |
| 4 | **N-01** | Ασφάλεια (P1) | Σύνθετο cached read model χωρίς entity/field authorization | Ο ίδιος τύπος κανόνα επιτρέπει δύο διαδρομές με διαφορετική προστασία, και το cache hit παρακάμπτει και τον handler. | Κλήση `authorizer.authorize('read', user)` πριν τη σύνθεση του DTO· regression με δύο principals διαφορετικών δικαιωμάτων. |
| 5 | **F-04** | Ασφάλεια (P1) | Replay idempotency χωρίς δέσμευση στο permission scope | Απαιτεί migration αποθηκευμένων records και σχέδιο namespace· γίνεται μετά τα μικρά correctness fixes. | Προαιρετικό `replayScopeFactory`, αποθηκευμένο `replayScope` σε Memory/Redis/PostgreSQL, fail-closed σύγκριση πριν από κάθε replay. Σταθερή ταυτότητα operation. |
| 6 | **F-09 + S-06** | Ορθότητα (P1) / επανάληψη (P2) | Κοινό optimistic delete και ενιαία απαίτηση autocommit | Τα δύο delete repositories διαφέρουν σε μία γραμμή. Κάθε διαδρομή που δηλώνει durable επιτυχία πρέπει να έχει το ίδιο όριο συναλλαγής. | `assertAutocommit(em, operation)` και `optimisticDelete(em, Entity, aggregate, entityName)` στο `ddd-core`· μετάβαση των repositories χρήστη/ρόλου. |
| 7 | **F-01** | Ορθότητα (P2) | Υπόλοιπο: πολιτική για adapters χωρίς revisions | Το versioned συμβόλαιο υλοποιήθηκε· απομένει η ρητή απόφαση για adapters που έχουν μόνο `get`/`set`. | Απόρριψη κατά τη διαμόρφωση ή πλήρης παράκαμψη cache για τη διαδρομή· test για «adapter χωρίς revisions». |
| 8 | **F-16** | Λειτουργικός θόρυβος (P2) | Πολιτική DLQ για αναμενόμενες απορρίψεις | Εξαρτάται από το F-04: το `IdempotencyCompletionError` μετά από επιτυχία δεν είναι επαναληπτέα εργασία. | Ταξινόμηση βάσει συγκεκριμένων κλάσεων σφάλματος κοντά στο composition root, όχι με υποσυμβολοσειρές μηνυμάτων. |
| 9 | **S-16** | Απλοποίηση (P1) | Τυποποιημένα tokens για το `context.items` | Αφαιρεί τα casts που επιτρέπουν σε key factories να παράγουν κλειδί με απόν principal. Προαπαιτούμενο για το S-10. | `createPipelineItem<T>`, `getPipelineItem`, `setPipelineItem`, `requirePipelineItem` πάνω στο υπάρχον Map, με διατήρηση ταυτότητας των exported symbols. |
| 10 | **S-09 / A-02 / A-03** | Απλοποίηση (P1) | Κοινές ρυθμίσεις logging και `loggerProvider` | Μειώνει επανάληψη με ήδη διαθέσιμα APIs, χωρίς νέα υποδομή. | Global `requestResponseLogLevel`, σύγκριση effective options ανά handler, σύνδεση `NativeLogger` μέσω `loggerProvider`. |
| 11 | **S-05 / A-05** | Απλοποίηση (P1) | Προεπιλεγμένο no-op `afterUpdate` | Μικρό ανεξάρτητο βήμα στο δικό μας `RootEntity`. | Μετατροπή σε concrete `protected afterUpdate(): void {}` και διαγραφή των κενών overrides. |
| 12 | **S-07 / A-07** | Ασφάλεια (P1) | Ασφαλή partitioned keys σε rate-limit και idempotency | Μειώνει επανάληψη σε όριο ασφάλειας tenant/principal και καταργεί το fallback `anonymous`. | Χρήση του υπάρχοντος rate-limit helper και νέος συμμετρικός idempotency helper με fail-closed ελέγχους και σχέδιο για τα αποθηκευμένα keys. |
| 13 | **S-03** | Απλοποίηση (P0) | Σύνθετος διακοσμητής `@PersistedWrite` | Μετά τη σταθεροποίηση των συμβολαίων F-01/F-09, ώστε να μην κωδικοποιηθεί λανθασμένο συμβόλαιο συναλλαγής. | Σύνθεση των τριών decorators στην κανονική σειρά, με προεπιλεγμένο entity extractor το πρώτο όρισμα. Τα επιμέρους APIs παραμένουν. |
| 14 | **F-11** | Συμβόλαιο (P2) | Διάκριση detachment από runtime immutability | Δεν είναι exploit· είναι λανθασμένη υπόσχεση στο τεκμηριωμένο συμβόλαιο. | Διατήρηση του deep detachment, διόρθωση της τεκμηρίωσης, προαιρετικό copy-on-read accessor, αυστηρή σειριοποίηση στα adapters μεταφοράς. |
| 15 | **F-14** | Συντήρηση (P2) | Ενοποίηση επιλογής tenant EntityManager | Διπλές αποφάσεις ασφαλείας κινδυνεύουν να αποκλίνουν ανά driver. | Ένας εσωτερικός resolver με ρητές εισόδους· κοινή table-driven σουίτα και για τους δύο adapters, με περιπτώσεις ενεργής συναλλαγής. |
| 16 | **S-13 → S-17** | Απλοποίηση (P2 → P1) | Σύνθεση metadata και presets πολιτικών | Το S-13 αλλάζει τη σημερινή συμπεριφορά overwrite, άρα γίνεται μόνο όταν χρειαστεί το S-17 ή πραγματικά stacked decorators. | Ένας μηχανισμός merge με τεκμηριωμένη σειρά και deduplication· μικρά app-owned presets που επεκτείνονται στα ίδια entries. |
| 17 | **S-10 / A-08** | Απλοποίηση (P1) | Core observation bag και κατάργηση `TelemetryBridgeBehavior` | Υπό όρους πραγματικής επαναχρησιμοποίησης· δεν πρέπει να δημιουργήσει μεγαλύτερο framework. | Μικρό ουδέτερο bag στο `IPipelineContext`, ίδια attributes και exported hooks, χωρίς έκθεση ευαίσθητων keys. |
| 18 | **S-04** | Απλοποίηση (P1) | `PublishDomainEventsBehavior` | Αναβολή μέχρι να αποδειχθεί ισοδυναμία: η αφαίρεση boilerplate δεν δικαιολογεί ρίσκο διπλής ή χαμένης δημοσίευσης. | Innermost global command behavior, ένας μόνο ιδιοκτήτης δημοσίευσης ανά handler, tests για retry, replay, nested commands και μεταβατική συνύπαρξη. |
| 19 | **S-11 / S-12** | Απλοποίηση (P2) | Στατικά globals σε async module· προεπιλογές hydration | Χρήσιμα αλλά όχι επείγοντα· εισάγουν κληρονομούμενες ρυθμίσεις που θέλουν σαφείς κανόνες. | Προαιρετική στατική ρύθμιση με καθορισμένο merge· προαιρετική hydrator policy στον `QueryRepository` με ρητή προτεραιότητα των method overrides. |
| 20 | **F-15 / S-18** | Συντήρηση (P2) | Διάσπαση bootstrap σε planning, execution και Nest integration | Μετά τη σταθεροποίηση ταυτότητας behavior και διαγνωστικών. Δεν τεκμηριώθηκε επιπλέον bootstrap bug. | Καθαρή `compilePipelinePlan`, ξεχωριστός runner, ένας Nest adapter για τα ιδιωτικά APIs· τα υπάρχοντα lifecycle tests ως characterization tests. |
