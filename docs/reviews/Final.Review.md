...
Ο παρακάτω πίνακας αποτυπώνει την επικαιροποιημένη σειρά εκτέλεσης των αρχιτεκτονικών αποφάσεων (D-01 έως D-03) και των εργασιών απλοποίησης (S-01 έως S-17), κατόπιν της **πλήρους ολοκλήρωσης της απόφασης D-02 (Option C: Own the Aggregate Primitive)**.

Η αυτονόμηση του `AggregateRoot` στο domain layer (με 0% εξαρτήσεις από NestJS/MikroORM) αναδιαμόρφωσε θεμελιωδώς τις υπόλοιπες εργασίες:
...
2. **S-04 (`PublishDomainEventsBehavior`):** Απλοποιήθηκε ριζικά. Το behavior λειτουργεί ως καθαρός αντάπτορας στο application layer: διαβάζει το αυτόνομο domain aggregate και διοχετεύει τα buffered events στο Nest `EventBus`, επιτρέποντας στους command handlers να απαλλαγούν από το `CommandBaseHandler` και την έγχυση του `EventBus`.
3. **D-01 (Transactional Outbox):** Αποσυνδέθηκε πλήρως από το Nest CQRS/EventPublisher. Τα uncommitted events είναι άμεσα προσβάσιμα στο persistence layer κατά το `save()`, επιτρέποντας την ατομική καταγραφή τους στον πίνακα `OutboxRecord` μέσα στο ίδιο DB transaction.
4. **S-14 (Καθαρισμός Δημόσιας Επιφάνειας):** Υλοποιήθηκε ήδη μερικώς, καθώς καταργήθηκε η διαρροή των re-exports του `@nestjs/cqrs` από το `ddd-core`.
5. **S-05 (`afterUpdate` hook):** Εφαρμόζεται άμεσα στο αυτόνομο `RootEntity` χωρίς καμία εξωτερική εξάρτηση.
...
| 2 | **D-03** | Αρχιτεκτονική Απόφαση (Gate) | Αυστηρή Ενθυλάκωση Aggregates vs Άμεση Χαρτογράφηση MikroORM Accessors | Απαιτείται επιβεβαίωση από τον Owner για τη διατήρηση του υφιστάμενου trade-off (hydration setters στο `RootEntity` και `accessor: true`). Εφόσον επιβεβαιωθεί, αποτρέπεται η ανάγκη δημιουργίας ξεχωριστών persistence mappers/records, ανοίγοντας το δρόμο για τα S-05, S-06 και S-03. | Διατήρηση των hydration setters στο `RootEntity` και των `accessor: true` στα schemas, με αυστηρή διατήρηση των linters/guards που απαγορεύουν τη χρήση setters στον επιχειρησιακό κώδικα. Εάν ζητηθεί αυστηρή ενθυλάκωση, απαιτείται σχεδιασμός Persistence Records πριν από αλλαγές στο domain. |
| 3 | **S-01** | Απλοποίηση (P0) | Καθολικός μηχανισμός παράκαμψης συμπεριφοράς (`@SkipPipeline`) | Θεμελιώδες δομικό στοιχείο (primitive) για τη διαχείριση pipeline. Επιτρέπει σε μεμονωμένους handlers να εξαιρούνται από global behaviors (π.χ. Audit) χωρίς hacky flags (`enabled: false`) ή διάσπαση modules. | Δημιουργία decorator `@SkipPipeline(...BehaviorTypes)`. Κατά το bootstrap, το `PipelineBootstrapService` αφαιρεί τα δηλωθέντα behaviors βάσει `BehaviorId` πριν από την επίλυση εξαρτήσεων. Αποτυχία στο bootstrap εάν υπάρχει ταυτόχρονα skip και local re-add. |
