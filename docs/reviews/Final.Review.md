...
| D-03 | Resolved via static guardrails (Retain Direct Mapping with Biome Grit Guard & @internal/@deprecated annotations). | ddd/core/domain/models/root.entity.ts; ddd/users-api/src/users/domain/models/user.entity.ts; ddd/users-api/src/roles/domain/models/role.entity.ts; biome/plugins/aggregate-identity.grit | Retained direct MikroORM aggregate mapping (`accessor: true`) for zero-boilerplate simplicity, with a syntax-only naming guard via the Biome Grit plugin (`biome/plugins/aggregate-identity.grit`) and `@internal`/`@deprecated` JSDoc annotations on all hydration setters. | Direct mapping retained. The Grit guard checks known hydration properties on receivers named user, role, aggregate, or entity, including literal bracket writes, compound assignments and updates. It cannot resolve types, aliases, dynamic keys, destructuring or reflection; domain-method mutation remains a code-review requirement. |
...
| 4 | **S-02** | Απλοποίηση (P0 - Ολοκληρώθηκε) | Τυποποιημένοι Intent Entry Builders (`authorize`, `rateLimit`, κ.λπ.) | **Ολοκληρώθηκε πλήρως:** Υλοποιήθηκαν τυποποιημένοι intent entry builders (`authorize`, `rateLimit`, `idempotent`, `cache`, `featureFlag`, `audit`, `resilience`) σε 7 πακέτα, με αυστηρούς compile-time τύπους που απαιτούν ρητά τα πεδία ενεργοποίησης (activation fields) στους handlers, διατηρώντας ταυτόχρονα τα raw tuples ως escape hatch. | Εξαγωγή των συναρτήσεων και των αντίστοιχων τύπων intent από τα package roots (`@nestjs-pipeline/*`). Επιστρέφουν έγκυρα `PipelineBehaviorEntry` tuples. Πλήρης κάλυψη με unit tests σε όλα τα πακέτα και επικαιροποίηση τεκμηρίωσης/READMEs. |
...
## Ανεξάρτητη προτεραιοποίηση μετά τον έλεγχο των τριών commits

Η παρακάτω πρόταση αφορά τα `ed3d5e2`, `5ed6657` και `50c479d` και συμπληρώνει τον προηγούμενο πίνακα. Κριτήριο είναι πρώτα η ορθότητα, μετά η μείωση πραγματικής επανάληψης και τελευταίες οι νέες αφαιρέσεις που χρειάζονται επιπλέον συντήρηση. Η σειρά αποτελεί σύσταση, όχι έγκριση αρχιτεκτονικών αλλαγών. Οι εργασίες D απαιτούν απόφαση του ιδιοκτήτη· όπου υπάρχει ήδη καταγεγραμμένη απόφαση, αξιολογείται η εφαρμογή της.

Το S-01 καταχωρίζει διαδρομή εκτέλεσης ανά εφαρμογή ακόμη και όταν δεν απομένει κανένα behavior. Τα tests καλύπτουν δύο Nest contexts με κοινό request-scoped handler και διαφορετική σειρά εκκίνησης/κλεισίματος. Το D-03 χρησιμοποιεί μόνο Grit: ελέγχει γνωστά ονόματα μεταβλητών και properties, όχι τύπους TypeScript. Επιτρέπει `dto.name` και απορρίπτει `role['name']`, compound assignments και increment/decrement. Άλλα ονόματα, aliases και δυναμικά keys παραμένουν εκτός κάλυψης.

| Σειρά | Εργασία | Θέση / πότε | Γιατί αυτή η προτεραιότητα | Σύντομο πώς |
|---|---|---|---|---|
| 1 | **S-01 — Εξαίρεση behaviors** | Υλοποιημένο με test απομόνωσης δύο εφαρμογών. | Η εκτέλεση behavior άλλης εφαρμογής παραβιάζει την απομόνωση και μπορεί να χρησιμοποιήσει ξένο tenant/context. | Καταχώριση σωστής διαδρομής για κάθε εφαρμογή ακόμη και με κενό τελικό pipeline. Πραγματικό test με δύο Nest contexts, κοινό scoped handler, διαφορετικά globals και κλείσιμο του ενός context. |
| 2 | **D-03 — Ενθυλάκωση aggregates** | Διατήρηση της επιλεγμένης χαρτογράφησης και συντακτικού ελέγχου Grit. | Τα δημόσια setters και ένας συντακτικός linter δεν ισοδυναμούν με πλήρη ενθυλάκωση. Ο κανόνας βασίζεται σε ονόματα και έχει τεκμηριωμένα όρια. | Διαχωρισμός DTOs από aggregates, κάλυψη bracket/compound assignments και αρνητικά tests. Τεκμηρίωση των ορίων· records/mappers μόνο αν ζητηθεί ισχυρότερη εγγύηση. |
| 3 | **D-01 — Αξιόπιστη παράδοση events** | Απόφαση νωρίς, όχι αυτόματη υλοποίηση outbox. | Η απαίτηση ανθεκτικότητας επηρεάζει τις επιλογές S-03/S-04. Το ανεξάρτητο AggregateRoot δεν λύνει την ατομικότητα. | Επιλογή απαιτούμενων events και εγγυήσεων. Αν απαιτείται outbox, σχεδιασμός κοινής συναλλαγής aggregate/outbox, acknowledgment μετά το commit και idempotent relay. Διαφορετικά διατήρηση της γνωστής in-memory συμπεριφοράς. |
| 4 | **D-02 — Ανεξάρτητο domain** | Αποδοχή της καταγεγραμμένης επιλογής, χωρίς επιπλέον διάσπαση τώρα. | Υπάρχει ήδη ιδιόκτητο AggregateRoot. Η αξία τώρα είναι να επιβεβαιωθούν τα όρια και η συμβατότητα, όχι να προστεθούν νέα πακέτα. | Καθαρό build και έλεγχος domain entry point χωρίς Nest/ORM runtime imports. Ρητή τεκμηρίωση τυχόν διαφοράς από το Nest EventPublisher και δοκιμή της εφαρμογικής δημοσίευσης. |
| 5 | **S-02 — Τυποποιημένες δηλώσεις πολιτικών** | Συμφωνώ, υψηλή αξία για καταναλωτές των πακέτων. | Εντοπίζει λανθασμένα options κατά τη μεταγλώττιση χωρίς νέο runtime μηχανισμό. | Μικροί builders ανά πακέτο που επιστρέφουν τα υπάρχοντα tuples, με υποχρεωτικά activation fields και type tests. Διατήρηση των raw tuples. |
| 6 | **S-15 — Διαγνωστικά εκκίνησης** | Συμφωνώ, σταδιακά μετά το S-02. | Καλύπτει JavaScript, δυναμική παραμετροποίηση και περιπτώσεις που οι τύποι δεν μπορούν να ελέγξουν. | Μικρό προαιρετικό contract ανά behavior, έλεγχος πραγματικών effective options και σαφή μηνύματα. Να διακρίνεται η σκόπιμα παθητική global δήλωση από την ελλιπή τοπική πολιτική. |
| 7 | **S-07 — Ασφαλή κλειδιά** | Συμφωνώ, πριν από διακοσμητικές απλοποιήσεις. | Μειώνει επανάληψη σε όριο ασφάλειας tenant/principal και αποτρέπει συγκρούσεις segments. | Χρήση του υπάρχοντος rate-limit helper και αντίστοιχου idempotency helper. Ρητή επιχειρησιακή ταυτότητα, fail-closed έλεγχοι και σχέδιο για αλλαγές στα αποθηκευμένα keys. Δεν απαιτεί προηγουμένως το S-16. |
| 8 | **S-06 — Κοινή optimistic διαγραφή** | Συμφωνώ, μικρή και ουσιαστική αλλαγή. | Ενοποιεί διπλότυπο κώδικα concurrency σε δύο repositories. | Helper με `id + expectedVersion`, refreshed διάγνωση και ελέγχους missing/conflict. Χωριστή αξιολόγηση κάθε αυστηρότερου transaction ή affected-row ελέγχου, ώστε να μην παρουσιαστεί ως απλή μεταφορά κώδικα. |
| 9 | **S-05 — Προεπιλεγμένο afterUpdate** | Συμφωνώ, μικρό ανεξάρτητο βήμα. | Αφαιρεί κενά overrides χωρίς νέα υποδομή. | Default no-op και αφαίρεση κενών υλοποιήσεων. Διατήρηση της συμβατότητας ορατότητας και του χρονισμού `@Mutate`, versions και events. |
| 10 | **S-08 — Περιττή σύνδεση pipeline cache στο sample** | Συμφωνώ μόνο μετά από επιβεβαίωση ότι παραμένει αχρησιμοποίητη. | Απλοποιεί το sample χωρίς να μειώνει δυνατότητες του δημόσιου πακέτου. | Αναζήτηση τρεχόντων consumers, αφαίρεση μόνο της περιττής εγγραφής/export στο ReliabilityModule και έλεγχος της ανεξάρτητης repository cache. |
| 11 | **S-09 — Κοινές ρυθμίσεις logging** | Συμφωνώ με διατήρηση του σημερινού αποτελέσματος. | Μειώνει επανάληψη με ήδη διαθέσιμα APIs. | Χρήση `loggerProvider`, μεταφορά μόνο πραγματικά κοινών defaults και σύγκριση effective options ανά handler για να μη μεγαλώσει απροσδόκητα ο όγκος logs. |
| 12 | **S-03 — PersistedWrite** | Συμφωνώ ως προαιρετική σύνθεση, μετά την απόφαση D-01. | Η ενιαία δήλωση μειώνει λάθη σειράς, αλλά το transaction/acknowledgment συμβόλαιο πρέπει πρώτα να είναι σαφές. | Σύνθεση υπαρχόντων decorators με tests για αποτυχία persistence, acknowledgment, CAS και barriers. Διατήρηση επιμέρους APIs και προσαρμογή των Grit guards στο νέο ισοδύναμο σχήμα. |
| 13 | **S-16 — Τυποποιημένα context items** | Συμφωνώ με μικρό API. | Μειώνει casts και λάθη keys, χωρίς να απαιτεί καθολική αναγραφή όλων των εφαρμογών. | Typed tokens/accessors πάνω στο υπάρχον Map, διατήρηση ταυτότητας των exported symbols και καθαρό σφάλμα για υποχρεωτική τιμή που λείπει. Χωρίς registry ή δεύτερο DI σύστημα. |
| 14 | **S-11 — Στατικά globals σε async module** | Συμφωνώ αν παραμένει η διπλή δήλωση providers. | Περιορισμένη βελτίωση DX που δεν χρειάζεται νέο μοντέλο πολιτικών. | Προαιρετική στατική ρύθμιση, έγκαιρη εγγραφή providers και συμβατό, καθορισμένο merge με το factory. Διατήρηση των σημερινών async μορφών. |
| 15 | **S-10 — Κοινές παρατηρήσεις telemetry** | Συμφωνώ υπό όρους πραγματικής επαναχρησιμοποίησης. | Το bridge είναι επαναλαμβανόμενη τεχνική σύνδεση, αλλά η αντικατάστασή του δεν πρέπει να δημιουργήσει μεγαλύτερο framework. | Μικρό ουδέτερο bag, αξιοποίηση S-16 όπου βοηθά, ίδια attributes και exported hooks. Χωρίς default έκθεση ευαίσθητων keys ή ανεξέλεγκτα metric labels. |
| 16 | **S-12 — Προεπιλογές hydration** | Χρήσιμο, όχι επείγον. | Μειώνει επανάληψη αλλά εισάγει κληρονομούμενες ρυθμίσεις που θέλουν σαφείς κανόνες. | Προαιρετική πολιτική repository, ρητή προτεραιότητα method overrides και tests ότι καμία cached ανάγνωση aggregate δεν επιστρέφει raw snapshot. |
| 17 | **S-13 — Σύνθεση metadata** | Μόνο όταν χρειαστεί το S-17 ή πραγματικά stacked decorators. | Αλλάζει τη σημερινή συμπεριφορά overwrite και αυξάνει την επιφάνεια συντήρησης. Δεν είναι προαπαιτούμενο για έναν απλό typed builder. | Ένας merge μηχανισμός με τεκμηριωμένη σειρά, deduplication και inheritance tests. Διατήρηση της συμπεριφοράς ενός μεμονωμένου UsePipeline. |
| 18 | **S-17 — Presets πολιτικών** | Μόνο μετά το S-13 και με επαναλαμβανόμενες πραγματικές αλυσίδες. | Χωρίς τέτοιες αλυσίδες προσθέτει δεύτερο τρόπο δήλωσης χωρίς επαρκές όφελος. | Μικρά app-owned presets που επεκτείνονται στα ίδια entries και ελέγχονται από S-15. Όχι κεντρικός decorator που γνωρίζει όλα τα addons. |
| 19 | **S-04 — Δημοσίευση events μέσω behavior** | Αναβολή μέχρι να αποδειχθεί καθαρό όφελος και ισοδυναμία. | Η αφαίρεση constructor boilerplate δεν αρκεί για να δικαιολογήσει ρίσκο διπλής ή χαμένης δημοσίευσης. Το ανεξάρτητο AggregateRoot δεν εξαλείφει αυτό το ρίσκο. | Απόφαση D-01 πρώτα· ένας μόνο ιδιοκτήτης δημοσίευσης ανά handler, innermost θέση και tests για retry, idempotency replay, αποτυχίες, nested commands και μεταβατική συνύπαρξη με τη base class. |
| 20 | **S-14 — Καθαρισμός δημόσιου API** | Μόνο στοχευμένα και με σαφή πολιτική συμβατότητας. | Οι εξωτερικοί καταναλωτές δεν αποτυπώνονται στη χρήση του sample. Αφαίρεση χρήσιμων helpers θα αναιρούσε τον στόχο της βιβλιοθήκης. | Ισοδύναμη επαναχρησιμοποίηση named types μπορεί να γίνει νωρίς. Αφαίρεση exports/aliases μόνο με συγκεκριμένο λόγο, migration και ελεγχόμενη breaking έκδοση. Διατήρηση adapters, builders και hooks. |
