...
other. Each layer must define its own consistency and invalidation requirements.
Existing barriers/CAS checks are mechanisms to verify, not proof of
...
Requires Node.js 22 or later. It brings `@cqrs-ddd/uuidv7` and
`@cqrs-ddd/safe-stringify`, which have no dependencies. Add `@mikro-orm/core` (7.x) only
if you use `/persistence` or the root barrel; `/domain`, `/application` and `/http` load
without it.
...
- [CreateUserHandler](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/cqrs/commands/create-user.handler.ts)
...
- [UpdateUserHandler](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/cqrs/commands/update-user.handler.ts)
...
- [Creation repository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/persistence/create-user.command-repository.ts)
- [Update repository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/persistence/update-user.command-repository.ts)
- [Deletion repository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/persistence/delete-user.command-repository.ts)
...
[GetUserQueryRepository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/persistence/get-user.query-repository.ts)
...
[GetUserHandler](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/cqrs/queries/get-user.handler.ts) authorizes
...
3. the running tenant scope, set with `runWithTenant(tenantId, fn)` from
   `@cqrs-ddd/core/application`, which is Node's `AsyncLocalStorage` and needs
   no framework.
...
`@cqrs-ddd/core` defines the `ICache<T>` interface and `CacheSetOptions`:
...
- Two runtime dependencies, both framework-neutral and dependency-free:
  `@cqrs-ddd/uuidv7` for aggregate, event and cache-barrier identifiers, and
  `@cqrs-ddd/safe-stringify` for cache-key serialization and escaping. No NestJS or
  `@nestjs-pipeline/*` package in any form.
- `@mikro-orm/core` is the only peer, and it is optional. `/persistence` and the
  ...
  extends MikroORM's `Type`. Import `@cqrs-ddd/core/domain` or
...
`@cqrs-ddd/core`. Concrete repositories retain their own `save()` logic:

```ts
import { AcknowledgePersisted, Cache, MapPersistenceErrors, PersistedWrite, optimisticUpdate } from '@cqrs-ddd/core/persistence';
```
...
See the [role repository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/roles/persistence/update-role.command-repository.ts)
and [user repository](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/src/users/persistence/update-user.command-repository.ts)
...
Run `pnpm --filter @cqrs-ddd/core test` for shared unit tests, including:
...
The application libSQL tests ([role](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/test/role-update-lifecycle.spec.ts)
and [user](https://github.com/aristoteliss/nestjs-pipeline/blob/master/api/test/user-update-lifecycle.spec.ts))
exercise actual ORM writes and real constraint diagnostics. Run users-api tests
with `pnpm --filter @nestjs-pipeline/ddd-api test`.

The lifecycle lint is implemented solely in the
[Grit plugin](https://github.com/aristoteliss/nestjs-pipeline/blob/master/biome/plugins/persistence-lifecycle.grit), configured by
