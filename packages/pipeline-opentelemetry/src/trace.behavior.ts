...
export class TraceBehavior implements IPipelineBehavior {
  async handle(
    ...
    return tracer.startActiveSpan(
      spanName,
      {
        ...
        attributes: this.buildSpanAttributes(context),
      },
      async (span) => {
        try {
  ...
  private buildSpanAttributes(context: IPipelineContext): Attributes {
    ...
    if (context.handlerName) {
      attributes['pipeline.handler.name'] = context.handlerName;
    }
